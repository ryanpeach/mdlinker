...
use crate::{config::Config, file::content::from_file, rules::duplicate_alias::DuplicateAlias};
        ...
        for file_path in files {
            ...
            for wikilink in wikilinks {
                ...
                if !lookup_table.contains_key(alias) {
                    let file_content =
                        std::fs::read_to_string(file_path.clone()).map_err(|e| miette!(e))?;
                    out.push(
                        BrokenWikilink::builder()
                            .id(format!("{CODE}::{alias}"))
                            .src(NamedSource::new(file_path.to_string_lossy(), file_content))
